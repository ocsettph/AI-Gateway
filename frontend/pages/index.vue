...
<template>
  <div class="md:min-h-[calc(100vh-11rem)] flex flex-col my-24 md:my-0 items-center justify-center container mx-auto relative">
    ...
    <div class="flex flex-col items-center gap-12 relative z-10 w-full px-4">
      ...
      <!-- Service Options -->
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 w-full max-w-5xl z-10">
        <div
          v-for="(service, index) in services"
          :key="`service-${index}`"
          class="group relative flex flex-col items-center justify-center transition-all rounded-xl shadow-lg border p-8 md:p-10"
          :class="{
            'rotate-1': index === 0,
            '-rotate-1': index === 2,
            'dark:!bg-[#1a1a1a] dark:hover:!bg-[#222222] bg-white hover:bg-gray-50 border-gray-200 dark:border-gray-800 cursor-pointer transform hover:scale-105': !service.comingSoon,
            'bg-gray-100 dark:!bg-[#0f0f0f] border-gray-300 dark:border-gray-700 cursor-not-allowed opacity-60': service.comingSoon,
          }"
        >
          <!-- Internal Link (NuxtLink) -->
          <NuxtLink
            v-if="!service.external && !service.comingSoon"
            :to="service.href"
          ...
          <!-- External Link (a tag) -->
          <a
            v-else-if="service.external && !service.comingSoon"
            :href="service.href"
            target="_blank"
          ...
          <div
            v-else-if="service.comingSoon"
            class="flex flex-col gap-4 items-center text-center w-full"
          >
            <!-- Icon -->
            <div :class="service.icon === 'N8n-logo-new.svg' ? 'w-20 h-20 md:w-24 md:h-24' : 'w-16 h-16 md:w-20 md:h-20'" class="flex items-center justify-center mb-2 opacity-50">
              <img 
                :src="base + 'assets/' + service.icon" 
                :alt="service.title" 
            ...
            <!-- Title -->
            <h2 class="text-xl md:text-2xl font-bold text-gray-500 dark:text-gray-500">
              {{ service.title }}
            </h2>

            <!-- Description -->
            <p class="text-sm md:text-base text-gray-500 dark:text-gray-500 break-words px-2">
              {{ service.description }}
            </p>

            <!-- Coming Soon Badge -->
            <span class="mt-2 px-4 py-1.5 bg-gray-300 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-xs md:text-sm font-semibold rounded-full">
              Coming Soon
            </span>
          </div>
        </div>
